...
// типы
public:
	typedef typename Ideal<_n, _O>::iterator iterator;
...
public:
	...
	struct
	{
		...
		const Monom<_n> lm1; //< старший моном многочлена
	};
	//! Второй многочлен
	...
	struct
	{
		...
		const Monom<_n> lm2; //< старший моном многочлена
	};
	//! НОК старших мономов
...
public:
	...
	void GetSPoly(MPoly<_n, _O>& spoly) const
	{
...
-#	В реализации использованы идеи оптимизации алгоритма Бухбергера,
	предложенные в работе [GMI]: R. Gebauer and H. M. Moller, 
	On an Installation of Buchberger’s Algorithm, J. Symb. Comp., 
	6: 257-286, 1987.
...
protected:
	typedef MPoly<_n, _O> _P;
	typedef Ideal<_n, _O> _I;
	typedef typename Ideal<_n, _O>::iterator _Iterator;
	struct _less : public std::binary_function <_Iterator, _Iterator, bool> 
	{
...
public:
	...
	template<class _O1>
	void Update(const MPoly<_n, _O1>& poly)
	{
	...
	template<class _O1>
	void Update(const Ideal<_n, _O1>& ideal)
	{
	...
	{	
		...
		while (_pairs.size())
		{
			...
			else if (Validate(spoly)) 
			{
				...
				_Update(pos);
				_stat.max_deg = MAX2(_stat.max_deg, spoly.Deg());
			}
			// трассировка
