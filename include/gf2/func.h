...
Для индексации значений функции используются числа от 0 до Size() - 1. 
Экземпляры WW<n> неявно приводятся к таким числам.
*******************************************************************************
*/
...
public:
	...
	/*! Создается система многочленов MI, которая описывает 
		действие функции. */
	template<class _O>
	void To(MI<_n + _m, _O>& MI) const
	{
		// очистить систему
		MI.SetEmpty();
